# Documentation de Silex

La documentation française est la source éditoriale de Silex. Choisissez le
parcours qui correspond à votre besoin.

- [Apprendre Silex](Learn/README.md) avec une progression structurée.
- [Comprendre le langage](Language/README.md), ses concepts et son
  comportement.
- [Réaliser une tâche](Guides/README.md) avec des guides pratiques et ciblés.
- [Utiliser les outils](Tools/README.md) pour construire, exécuter, tester et
  maintenir un projet.
- [Trouver un contrat exact](Reference/README.md) dans la référence du langage
  implémenté.

[Read in English](../EN/README.md)
