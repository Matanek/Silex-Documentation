# Modéliser des données

Choisissez la forme qui correspond à l'identité, à la durée de vie et aux
variations de vos données.

- [Créer une valeur nominale avec une structure](Structures.md)
- [Partager une identité avec une classe](Classes.md)
- [Spécialiser une classe par héritage](Inheritance.md)
- [Définir des membres et des conteneurs statiques](Static-members.md)
- [Adapter un type à plusieurs types concrets](Generics.md)
- [Exécuter un nettoyage déterministe](Cleanup.md)
- [Définir un contrat avec un protocole](Protocols.md)
- [Ajouter des méthodes avec une extension](Extensions.md)
- [Représenter une valeur absente](Optionals.md)
- [Représenter un choix avec un enum et `match`](Enums.md)
- [Regrouper des valeurs avec un tuple](Tuples.md)
- [Donner un autre nom à un type](Type-aliases.md)
- [Observer les métadonnées d'une valeur](Reflection.md)

[Revenir aux concepts du langage](../README.md)
